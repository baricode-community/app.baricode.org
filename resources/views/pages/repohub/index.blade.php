<x-layouts.base :title="__('RepoHub — Koleksi Repo Keren')">
    ...
    <div class="relative py-20 px-4 overflow-hidden">
        ...
        <div class="relative z-10 max-w-4xl mx-auto text-center">
            ...
            <!-- Search -->
            <form action="{{ route('repohub.index') }}" method="GET" class="flex gap-3 max-w-lg mx-auto">
                @if($activeTag)
                ...
                />
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl font-semibold hover:from-purple-500 hover:to-indigo-500 transition">
                    Cari
                </button>
                @if($search || $activeTag)
                    <a href="{{ route('repohub.index') }}" class="px-4 py-3 bg-white/10 border border-white/20 rounded-xl hover:bg-white/20 transition text-gray-300 flex items-center">✕</a>
                @endif
            </form>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 pb-20">
        ...
        @if($tags->count() > 0)
            <div class="flex flex-wrap gap-2 mb-8">
                <a href="{{ route('repohub.index', $search ? ['q' => $search] : []) }}"
                    class="px-4 py-2 rounded-full text-sm font-medium border transition
                        {{ !$activeTag ? 'bg-purple-600 border-purple-600 text-white' : 'bg-white/5 border-white/20 text-gray-300 hover:border-purple-500 hover:text-white' }}">
                    Semua
                </a>
                @foreach($tags as $tag)
                    <a href="{{ route('repohub.index', array_filter(['tag' => $tag->slug, 'q' => $search])) }}"
                        class="px-4 py-2 rounded-full text-sm font-medium border transition
                            {{ $activeTag?->id === $tag->id ? 'bg-purple-600 border-purple-600 text-white' : 'bg-white/5 border-white/20 text-gray-300 hover:border-purple-500 hover:text-white' }}">
                        {{ $tag->name }}
                    </a>
                @endforeach
            </div>
        ...
        @else
            <div class="text-center py-20">
                ...
                @if($search || $activeTag)
                    <a href="{{ route('repohub.index') }}" class="mt-4 inline-block text-purple-400 hover:text-purple-300 transition">Lihat semua repo</a>
                @endif
            </div>
        @endif
    </div>
